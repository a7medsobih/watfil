...
> آخر تحديث: كتالوج واتفل العام + مقارنة المنتجات + إحصائيات الموقع (cache) + إعجابات موحّدة `/customer/likes` + Billboards + مشاركة المتجر + طلبات انضمام الشركات + Perks (ضمان/هدية) + `watfil_commission` + صور الهوية و service presets.
...
- صفحة تفاصيل الشركة.
- عرض التغطية والمنتجات والإحصائيات.
...
- مناسب للـ homepage / about.
- متخزّن في السيرفر (`WATFIL_STATISTICS_CACHE_MINUTES`, افتراضي 5 دقايق).
- هيدر: `Cache-Control: public, max-age=...` — الأرقام تقريبية وليست لحظية.

---

### 2.18 كتالوج منتجات واتفل (Supplier Products)

```http
GET /api/public/products
GET /api/public/products/{supplierProduct}
GET /api/public/products/{supplierProduct}/companies?governorate_id=
GET /api/public/products/{supplierProduct}/similar
```

Query شائعة للقائمة:

```text
governorate_id?, city_id?, category_id?, parent_category_id?,
product_type_id?, product_type?, search?, min_price?, max_price?, page?, per_page?
```

Notes:

- منتجات ظاهرة + عليها شركة مؤهلة (`offering_companies_count`).
- مع customer token يظهر `is_liked`.
- بعد اختيار منتج: اعرض الشركات العارضة حسب المحافظة ثم ادخل متجر الشركة.

نسخ بنفس الـ payload تحت تسجيل العميل (Bearer مطلوب):

```http
GET /api/customer/products
GET /api/customer/products/{id}
GET /api/customer/products/{id}/companies
GET /api/customer/governorates
GET /api/customer/companies
GET /api/customer/companies/top-rated
GET /api/customer/companies/{company}
GET /api/customer/companies/{company}/products
GET /api/customer/companies/{company}/product-details
GET /api/customer/companies/{company}/billboards
```

---

### 2.19 مقارنة منتجات الكتالوج

```http
GET /api/public/products/compare?product_ids[]=1&product_ids[]=2
GET /api/customer/products/compare?product_ids[]=1&product_ids[]=2
```

Rules:

- بالظبط **منتجين** مختلفين (`product_ids` size=2 + distinct).
- لو أحدهم غير ظاهر/بدون شركات عارضة → `404`.
- Response: `{ data: { products: [PublicCatalogProduct, PublicCatalogProduct] } }` بنفس ترتيب الـ IDs.
- التقسيط/المميزات التفصيلية ليست على مستوى الكتالوج (`has_installment: false`).

---

### 2.20 أعلى الشركات تقييمًا

```http
GET /api/public/companies/top-rated
GET /api/customer/companies/top-rated
```

لشرائح homepage / اكتشاف الشركات الأفضل تقييمًا.

---

### 2.21 لوحات إعلانية داخل متجر الشركة (Billboards)

```http
GET /api/public/companies/{company}/billboards
GET /api/customer/companies/{company}/billboards
```

كل عنصر يشير عادة لمنتج كتالوج:

```ts
{
  id, image, sort_order, supplier_product_id,
  product: { id, name, image, cash_price } | null,
  target: { type: "supplier_product", product_id, path: "/public/products/{id}" }
}
```

عند الضغط: افتح `target.path` أو تفاصيل المنتج.

إدارة الأدمن: `/api/super-admin/billboards` (§10.4).

---

### 2.22 رابط مشاركة متجر الشركة

```http
GET /api/public/store/{taxNumber}
```
...
{
  data: {
    company_id, tax_number, name, logo, about,
...
تدفق الويب: جرّب `deep_link` → لو التطبيق مش متثبت خلال ~1.5ث روح Play Store أو `web_url`.

صور الهوية تُدار من الشركة: `/api/company/identity-images` (§10.5).

---

### 2.23 طلب انضمام شركة (من الموقع)
...
- `201` عند النجاح.
- الرقم الضريبي/الهاتف لا يتكرران مع شركة موجودة أو طلب معلّق.
- مراجعة الأدمن: `/api/super-admin/company-join-requests`.
...
### 3.15–3.16 إعجابات العميل الموحّدة (Companies + Products)

> **Breaking change:** مسارات `.../like` القديمة اتشالت.
> استخدم فقط `POST/DELETE /api/customer/likes`.

```http
POST   /api/customer/likes
DELETE /api/customer/likes
GET    /api/customer/likes
```

**Auth:** Bearer توكن عميل فقط.
...
|------|-------------|---------------|
| `company` | رقم الشركة | غير مطلوب |
| `company_product` | منتج شركة | **مطلوب** |
| `catalog_product` | منتج كتالوج واتفل | غير مطلوب |

- `DELETE` يقبل نفس الحقول في body أو query string.
- تكرار الإعجاب / إلغاء بدون إعجاب سابق → `422`.
- Response: `data.type` + `data.company` أو `data.product` مع `is_liked` و`likes_count`.

#### قائمة الإعجابات

```http
GET /api/customer/likes?source=company|catalog&products_page=1&companies_page=1
```
...
{
  data: {
    products: Array<{ liked_at: string; source: "company"|"catalog"; product: object }>;
    companies: Array<{ liked_at: string; company: object }>;
  },
  meta: { products: PaginationMeta; companies: PaginationMeta }
}
```

تفاصيل إضافية: `docs/WATAFL_CUSTOMER_LIKES_FRONTEND.md`.
...
### 3.18ب إعجاب المنتجات (موحّد)

تم دمجه في §3.15–3.16.

- منتج شركة: `{ "type": "company_product", "id": productId, "company_id": companyId }`
- منتج كتالوج: `{ "type": "catalog_product", "id": supplierProductId }`

`likes_count` / `is_liked` يظهران في القوائم العامة عند إرسال customer token.
...
- إنشاء/قبول/سحب الإحالة من واجهات الشركة (§4.36).
...
- أنواع الشركة الحالية المسموحة: **`warranty`** و **`gift`** فقط (`COMPANY_TYPES`).
  - `warranty`: يتطلب `perk_preset_id` + `warranty_years` (عدد سنوات).
  - `gift`: preset **أو** عنوان مخصص؛ `warranty_years` ممنوع مع gift.
- Response الميزة يتضمن `warranty_years` عند الضمان.
...
- `watfil_commission` مبلغ ثابت (≥ 0) يظهر للشركة في `catalog/available` و`catalog/mine` (§10.3).
- تعطيل/إلغاء تفعيل المنتج يمنعه من الظهور في الكتالوج العام والمتاح للشركات.
- خطط التقسيط لمنتجات الكتالوج تُدار عادة على مستوى الشركة (`catalog/{id}/installment-plans`) وليست خطة عامة مشتركة بالضرورة.
...
1. السوبر أدمن ينشئ قائمة مميزات جاهزة من `/api/super-admin/perk-presets` (أنواع الشركة: `warranty` / `gift`).
2. الشركة تجلب القائمة من `/api/company/perk-presets?type=warranty|gift`.
3. تضيف ميزة لمنتج خاص أو منتج كتالوج:
   - ضمان: preset + `warranty_years`
   - هدية: preset أو عنوان مخصص
4. الميزة النشطة تظهر مباشرة للعميل في عروض المتجر عبر `has_perks` / `perks` / `warranty_years`.
...
- مقارنة المنتجات موجودة: `GET /api/public|customer/products/compare` (§2.19).
- لا توجد دورة طلب تقسيط كاملة بالمستندات والعرض النهائي؛ الموجود هو خطط تقسيط على المنتج وsnapshot داخل الطلب.
- كتالوج واتفل العام موجود بالكامل: قائمة + تفاصيل + شركات عارضة + مشابه + مقارنة (§2.18–2.19).
- إعجابات العميل موحّدة على `/api/customer/likes` (§3.15–3.16) — المسارات القديمة `.../like` اتشالت.
- إحصائيات الموقع العامة `/api/public/statistics` مع cache قصير (§2.17).
- Billboards + روابط المتجر + طلبات الانضمام موثّقة في §2.21–2.23 و§10.
...
- في الإعجابات: استخدم فقط `POST/DELETE /api/customer/likes` مع `type` + `id` (+ `company_id` لمنتج الشركة)، وحدّث من `data.product` أو `data.company`.
- في المقارنة: اجمع منتجين كتالوج ثم نادِ `/products/compare?product_ids[]=`.
- في الـ homepage: استخدم `/public/statistics` (Cached) + `companies/top-rated` + كتالوج `/public/products`.
- في مشاركة المتجر: `/public/store/{taxNumber}` ثم deep link / Play Store من الكلاينت.
...
| Product Perks (عام)                  | §2.9 + §2.13          |
| Customer Points & Rewards            | §3.21 – §3.25         |
| Customer Unified Likes               | §3.15–3.16            |
| Public Catalog + Compare             | §2.18–2.19            |
| Site Statistics (cached)             | §2.17                 |
| Billboards + Store Share + Join Req | §2.21–2.23 + §10      |
...
- **Engagement الشركة:** §4.37 + §3.15–3.16
- **كتالوج/مقارنة/إحصائيات/مشاركة:** §2.17–2.23 + §10
...
- في الإعلانات: سجّل impression عند العرض واستخدم redirect link للنقر.
- في استبدال النقاط: اعرض `pending_balance` من محفظة النقاط أثناء المراجعة.
- في المميزات: اعرض `has_perks`/`perks` (ومعها ضمان/هدايا)، ووفّر UI اختيار preset؛ للضمان اطلب `warranty_years`.
...
- حدّث أزرار القلب للـ likes الموحّد فورًا (`POST/DELETE /customer/likes`).
- شاشة Favorites = `GET /customer/likes`.
- قارن منتجات كتالوج فقط عبر `/products/compare`.
- homepage: `/public/statistics` + `/public/products` + `/public/companies/top-rated`.
- مشاركة متجر: `/public/store/{taxNumber}` ثم deep link / Play Store من الكلاينت.

---

## 10. تحديثات إضافية (كتالوج · مشاركة · Billboards · Perks · عمولة)

### 10.1 دورة تصفح الكتالوج (Website / App)

1. `GET /public/statistics` + `GET /public/governorates`
2. `GET /public/product-types` + `GET /public/categories`
3. `GET /public/products?governorate_id=&...`
4. `GET /public/products/{id}`
5. `GET /public/products/{id}/companies?governorate_id=`
6. اختياري: مقارنة `/products/compare?product_ids[]=a&product_ids[]=b`
7. دخول متجر الشركة: `/public/companies/{id}` + منتجات + billboards

نفس التدفق تحت `/api/customer/*` بعد login لإظهار `is_liked` / `my_rating`.

---

### 10.2 Perks — ضمان وهدايا

أنواع الشركة المسموحة: `warranty` | `gift`.

- الضمان: يحتاج `perk_preset_id` + `warranty_years`
- الهدية: preset **أو** عنوان مخصص؛ `warranty_years` ممنوع مع gift

Endpoints الشركة:

```http
GET  /api/company/perk-presets?type=warranty|gift
CRUD /api/company/products/{companyProduct}/perks
CRUD /api/company/catalog/{supplierProduct}/perks
```

أدمن presets: `/api/super-admin/perk-presets`.

---

### 10.3 عمولة واتفل على منتج المورد

حقل `watfil_commission` (مبلغ ثابت ≥ 0) على `supplier_products`.

- الأدمن يدخله عند إنشاء/تعديل منتج مورد.
- يظهر للشركة في كتالوج المتاح/المضاف (`catalog/available` و`catalog/mine`).
- التخزين/العرض جاهز؛ ربطه داخل حساب عمولة الطلب يعتمد على تفعيل لاحق إن لزم.

---

### 10.4 إدارة Billboards (سوبر أدمن)

```http
GET    /api/super-admin/billboards
POST   /api/super-admin/billboards
GET    /api/super-admin/billboards/{billboard}
POST   /api/super-admin/billboards/{billboard}
DELETE /api/super-admin/billboards/{billboard}
PATCH  /api/super-admin/billboards/{billboard}/toggle-status
GET    /api/super-admin/billboards/companies-for-targeting
```

Audience: كل المتاجر أو متاجر محددة. العرض العام عبر §2.21.

---

### 10.5 صور هوية المتجر + خدمات جاهزة (شركة)

```http
GET    /api/company/identity-images
POST   /api/company/identity-images
POST   /api/company/identity-images/replace
DELETE /api/company/identity-images/{identityImage}

GET  /api/company/service-presets
POST /api/company/profile/services
```

- حد أقصى عادة 3 صور هوية (تظهر في رابط المشاركة §2.22).
- قائمة الخدمات ملك الأدمن (`/api/super-admin/service-presets`)؛ الشركة تختار فقط عبر `service_preset_id`.

---

### 10.6 طلبات انضمام الشركات (أدمن)

```http
GET   /api/super-admin/company-join-requests
GET   /api/super-admin/company-join-requests/{id}
PATCH /api/super-admin/company-join-requests/{id}/mark-processed
```

الإنشاء العام في §2.23.
